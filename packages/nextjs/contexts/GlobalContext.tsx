...
export interface MasterConfigurationState {
  ...
  minRentCost: number;
  sendTransaction: (transaction: Transaction | VersionedTransaction) => Promise<void>;
...
export const GlobalContext = createContext<MasterConfigurationState>({} as MasterConfigurationState);
...
export const GlobalContextProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [isTransactionSheetOpen, setIsTransactionSheetOpen] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    // console.log("local storage:", currentValue);
    ...
    if (currentValue !== base58.encode(burner.secretKey)) {
      localStorage.setItem(LOCAL_STORAGE_BURNER_KEY, base58.encode(burner.secretKey));
  ...
  const prepareTransaction = useCallback(
    ...
    [setIsTransactionSheetOpen, isTransactionSheetOpen, loading, setLoading, sendTransaction, burner],
  );
  ...
  }, [transaction, isTransactionSheetOpen, setIsTransactionSheetOpen, setLoading]);
